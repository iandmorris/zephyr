...
socket and defines the ``mikrobus_serial`` and ``mikrobus_header`` node labels(see :ref:`shields` for more details).

The DA14531 Module contained on the shield must programmed with a binary file
that supports the HCI interface over UART, without hardware flow control as
these signals are not support on the Click footprint.
