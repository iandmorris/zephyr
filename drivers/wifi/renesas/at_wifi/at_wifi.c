...
K_KERNEL_STACK_DEFINE(at_wifi_workq_stack,
			CONFIG_WIFI_AT_WIFI_WORKQ_STACK_SIZE);
 ...
 *
 * Scan response is formatted as follows:
...
MODEM_CMD_DIRECT_DEFINE(on_cmd_scan_rsp)
{
	...
	len = net_buf_linearize(dev->scan.rsp_buf, sizeof(dev->scan.rsp_buf) - 1,
							data->rx_buf, 0, sizeof(dev->scan.rsp_buf) - 1);
	...
	*end = '\0';
	...
	do {
		...
		*next = '\0';
		...
		dev->scan.last_field = ++next;
		if (next == end) {
			return -EAGAIN;
		}
	} while (*next != '\r'); /* Last response is followed by <CR><LF> */

	// TODO - make sure we don't consume the OK part of the response so that the caller can parse it...
	return len;
}
...
MODEM_CMD_DIRECT_DEFINE(on_cmd_wifi_state)
{
	...
	len = net_buf_linearize(wfstat_buf, sizeof(wfstat_buf) - 1,
				data->rx_buf, 0, sizeof(wfstat_buf) - 1);
...
MODEM_CMD_DEFINE(on_cmd_get_mac_addr)
{
	...
	net_bytes_from_str(dev->info.mac_addr,
						sizeof(dev->info.mac_addr), argv[0]);
...
*/
	...
	char cwjap_buf[64]; // TODO - use macro or create dummy string of max length
	struct in_addr ip_addr;
	...
	if (result == 1) {
		...
		*next = '\0';
		...
		result = WIFI_STATUS_CONN_SUCCESS;
	} else {
...
static void at_wifi_init_work(struct k_work *work)
{
	...
	ret = modem_cmd_handler_setup_cmds(&dev->modem.ctx.iface,
					   &dev->modem.ctx.cmd_handler, setup_cmds,
	...
	net_if_set_link_addr(dev->net_iface, dev->info.mac_addr,
			     sizeof(dev->info.mac_addr), NET_LINK_ETHERNET);
...
static void at_wifi_mgmt_scan_work(struct k_work *work)
{
	...
	static const struct modem_cmd cmd =
		MODEM_CMD_DIRECT("+WFSCAN:", on_cmd_scan_rsp);
	...
	ret = modem_cmd_send(&dev->modem.ctx.iface, &dev->modem.ctx.cmd_handler,
						 ...
						 &cmd, 1, "AT+WFSCAN", &dev->sem_response,
...
static void at_wifi_mgmt_connect_work(struct k_work *work)
{
	...
	static const struct modem_cmd cmd =
		MODEM_CMD("+NWIP:", on_cmd_nwk_ip, 4U, ",");
			     ...
			     struct wifi_iface_status *status)
{
	...
	static const struct modem_cmd wfstat_cmd =
		MODEM_CMD_DIRECT("+WFSTAT:", on_cmd_wifi_state);

	static const struct modem_cmd wfrssi_cmd =
		MODEM_CMD("+RSSI:", on_cmd_wifi_rssi, 1U, "");
	...
	ret = modem_cmd_send(&data->modem.ctx.iface, &data->modem.ctx.cmd_handler,
						 &wfstat_cmd, 1, "AT+WFSTAT", &data->sem_response,
					     AT_WIFI_CMD_TIMEOUT);
	if (ret) {
		return ret;
	}
				 ...
				 struct wifi_version *params)
{
	...
	/* Version information should have been read during the initialization
	   process */
	if ((strlen(data->info.sdk_ver_str) == 0) &&
	    (strlen(data->info.ver_str) == 0)) {
...
static void at_wifi_rx(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		modem_iface_uart_rx_wait(&data->modem.ctx.iface, K_FOREVER);
		modem_cmd_handler_process(&data->modem.ctx.cmd_handler,
				&data->modem.ctx.iface);
...
static int at_wifi_init(const struct device *dev)
{
	...
	ret = modem_cmd_handler_init(&data->modem.ctx.cmd_handler,
					 &data->modem.cmd_handler_data,
				     &cmd_handler_config);
	if (ret < 0) {
		return ret;
	}
...
//		.hw_flow_control = DT_PROP(ESP_BUS, hw_flow_control),
	...
	ret = modem_iface_uart_init(&data->modem.ctx.iface,
