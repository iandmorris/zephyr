...
#include <zephyr/kernel.h>  // TODO - is this needed?
...
#define AT_WIFI_DRIVER_VERSION  KERNEL_VERSION_STRING
...
#if 0
/*
DA16200 can return these (and possibly more):
WPA: Indicates that the network supports WPA (Wi-Fi Protected Access).
WPA2: Indicates support for WPA2, which is an improvement over WPA.
RSN: Signifies that the network uses Robust Security Network (RSN), a security standard for WPA2.
IBSS: Indicates that the network is an ad-hoc (IBSS) network.
ESS: Denotes that the network is an extended service set, meaning it has multiple access points.
PSK: Indicates that the network uses Pre-Shared Key authentication.
CCMP: Indicates that the network supports CCMP (Counter Mode with Cipher Block Chaining Message Authentication Code Protocol) for encryption.
TKIP: Indicates that the network uses TKIP (Temporal Key Integrity Protocol).
WEP: Indicates that the network uses WEP (Wired Equivalent Privacy), which is less secure.
QOS: Indicates support for Quality of Service features.
WPA3: Indicates that the network supports WPA3.
SAE: Stands for Simultaneous Authentication of Equals, the primary authentication method used in WPA3.

must be mapped to Zephyr security type:
...
MODEM_CMD_DEFINE(on_cmd_sdk_version)
{
	...
	// TODO can this be less than or EQUAL to?
...
int at_wifi_mgmt_get_version(const struct device *dev, struct wifi_version *params)
{
	...
	/* SDK version information should have been read during the initialization
	   process */
	params->drv_version = AT_WIFI_DRIVER_VERSION;
...
static int at_wifi_init(const struct device *dev)
{
	...
	// TODO - perform software reset (AT+RESTART) and wait for response (INIT) as no hardware reset available on DA16xxx...
