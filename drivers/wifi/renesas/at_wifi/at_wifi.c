...
MODEM_CMD_DEFINE(on_cmd_wifi_state)
{
	...
	char *curr;
	char *next;
	char wfstat_buf[64]; // TODO - use macro or create dummy string of max length
	...
	/* Find end of first parameter (result) so we know it's
	   all in the buffer before attempting to parse it. */
	next = strchr(curr, ',');
	if (next == NULL) {
		return -EAGAIN;
...
//		SETUP_CMD("AT+WFSTA", "", on_cmd_wifi_status, 1U, ""),
		SETUP_CMD("AT+WFSTAT", "+WFSTAT:", on_cmd_wifi_state, 0U, ""),
	};

	dev = CONTAINER_OF(work, struct at_wifi_data, iface_status_work);

	ret = modem_cmd_handler_setup_cmds(&dev->modem.ctx.iface,
					   &dev->modem.ctx.cmd_handler, query_status_cmds,
